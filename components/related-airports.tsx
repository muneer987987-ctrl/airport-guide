...
export default async function RelatedAirports({
  ...
  // If less than 3 from same country, fetch from other countries
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {(relatedAirports as any[]).map((airport: any) => (
          <Link
            key={airport.id}
          ...
          >
            <p className="font-display font-600 group-hover:text-signal transition-colors">
              {airport.name} ({airport.iataCode || airport.iata_code || airport.code || ""})
            </p>
